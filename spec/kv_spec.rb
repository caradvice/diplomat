    ...
    describe "#get" do
      ...
      context "ACLs NOT enabled" do
        it "GET" do
          ...
          faraday.stub(:get).and_return(OpenStruct.new({ status: 200, body: json }))
          kv = Diplomat::Kv.new(faraday)
          expect(kv.get(key)).to eq([{ key: key, value: "toast" }])
        end
        it "GET with consistency param" do
          ...
          faraday.stub(:get).and_return(OpenStruct.new({ status: 200, body: json }))
          kv = Diplomat::Kv.new(faraday)
          expect(kv.get("key", options)).to eq([{ key: key, value: "toast" }])
        end
      end
      context "ACLs enabled, without valid_acl_token" do
        it "GET with ACLs enabled, no valid_acl_token" do
          ...
          faraday.stub(:get).and_return(OpenStruct.new({ status: 200, body: json }))
          kv = Diplomat::Kv.new(faraday)
          expect(kv.get(key)).to eq([{ key: key, value: "Faraday::ResourceNotFound: the server responded with status 404" }])
        end
        it "GET with consistency param, without valid_acl_token" do
          ...
          faraday.stub(:get).and_return(OpenStruct.new({ status: 200, body: json }))
          kv = Diplomat::Kv.new(faraday)
          expect(kv.get("key", options)).to eq([{ key: key, value: "Faraday::ResourceNotFound: the server responded with status 404" }])
        end
      end
